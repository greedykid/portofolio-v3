...
export default function ThemeToggleButton() {
  ...
  return (
    <button
      ...
      title={isDark ? 'Mode Gelap (Klik untuk ganti)' : 'Mode Terang (Klik untuk ganti)'}
    ...
    >
      {isDark ? (
        <FiMoon className="h-5 w-5 text-indigo-400 transition-transform duration-300 group-hover:-rotate-12 group-hover:scale-110" />
      ) : (
        <FiSun className="h-5 w-5 text-amber-500 transition-transform duration-300 group-hover:rotate-45 group-hover:scale-110" />
      )}
    </button>
  );
}
